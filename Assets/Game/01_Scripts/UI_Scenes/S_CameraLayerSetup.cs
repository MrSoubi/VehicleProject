...
public class S_CameraLayerSetup : MonoBehaviour
{
    ...
    public void SetPlayerID(int playerID)
    {
        ...
        _CameraUI.cullingMask &= ~((1 << 7) | (1 << 8) | (1 << 9) | (1 << 10)); //Enleve les layers des joueurs sur le culling mask de la camera
        _CameraUI.cullingMask |= LayerMask.GetMask($"PlayerCam{_playerID + 1}"); //Rajoute le layer du joueur

        _speedLines.gameObject.layer = _layerMask;
